...
class Command(BaseCommand):
    ...
    Es idempotente: solo crea datos si la BD está vacía.
    Se lanza automáticamente desde build.sh durante el deploy en Render.
    """
    help = 'Siembra las categorías y cursos iniciales en la base de datos (solo si está vacía).'

    def handle(self, *args, **kwargs):
        # Si ya hay categorías, no hacemos nada (idempotente)
        if Category.objects.exists():
            self.stdout.write(self.style.WARNING(
                '⚠️  La base de datos ya tiene datos. Omitiendo siembra.'
            ))
            return

        self.stdout.write('🌱 Sembrando base de datos de producción...')
        ...
        self.stdout.write(self.style.SUCCESS(
            '✅ Base de datos cósmica poblada con éxito. '
            f'{Category.objects.count()} categorías, {Course.objects.count()} cursos.'
        ))
